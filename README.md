# PLEASE MAIN TUTORIAL FROM THE AUTHOR IS BELOW, BUT GO AHEAD AND READ THIS SUMMARIZED SCRIPT TO RUN B-I-T-B ON VPS/Linux (Ubuntu 22.4 and 24.0)
# After setting up your EvilGinx (MAKE SURE TO TEST EVILGINX LINK IS WORKING BEFORE PROCEEDING WITH THESE)


The HTML Pages has been edited to a custom (Sophos IT meeting)

Setting Up Browser-In-The-Browser Attack. 

After setting up your EvilGinx (MAKE SURE TO TEST EVILGINX LINK IS WORKING BEFORE PROCEEDING WITH THESE)

Now come here...To setup Browser-In-The-Browser Attack.

Go to your evilginx config file located /root/.evilginx/config.json 
Change the https port from 433 to 8443 so Evilginx and Apache can have separate listening port and avoid breakings.
Then change the DNS port from port 53 to port 5300
...
Additional: Optional: To set the Calendly widget to use your account instead of the default I have inside, go to pages/primary/script.js and change the CALENDLY_PAGE_NAME and CALENDLY_EVENT_TYPE
